...
function exportLibraryMarkdown() {
  const shelfBooks = books.filter(b => b.status !== 'not-owned');
  const byStatus = { reading: [], read: [], unread: [] };
  shelfBooks.forEach(b => { if (byStatus[b.status]) byStatus[b.status].push(b); });

  const statusLabel = { reading: 'Currently Reading', read: 'Read', unread: 'Unread' };
  ...
  const bookLine = b => {
    ...
    if (b.status === 'reading' && b.total_pages > 0) {
      const pct = Math.min(100, Math.round((b.pages_read || 0) / b.total_pages * 100));
      parts.push(`· ${pct}% read`);
    }
    return `- ${parts.join(' ')}`;
  };

  let md = `# My Tsundoku Library\n\n`;
  md += `*Exported on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}*\n\n`;
  md += `**${shelfBooks.length} books total** — `;
  md += `${byStatus.reading.length} reading · ${byStatus.read.length} read · ${byStatus.unread.length} unread\n\n`;
  md += `---\n\n`;

  ['reading', 'read', 'unread'].forEach(s => {
    if (!byStatus[s].length) return;
    const sorted = [...byStatus[s]].sort((a, b) => (a.title || '').localeCompare(b.title || ''));
    md += `## ${statusLabel[s]} (${sorted.length})\n\n`;
    sorted.forEach(b => { md += bookLine(b) + '\n'; });
    md += '\n';
  });
  ...
  shelfBooks.forEach(b => {
    ...
    authorMap.get(a).push(b.title);
  });
  const sortedAuthors = Array.from(authorMap.entries()).sort((a, b) => a[0].localeCompare(b[0]));

  if (sortedAuthors.length) {
    md += `---\n\n## Authors (${sortedAuthors.length})\n\n`;
    sortedAuthors.forEach(([author, titles]) => {
      md += `- **${author}** — ${titles.length} ${titles.length === 1 ? 'book' : 'books'}`;
      if (titles.length <= 4) md += `: ${titles.join(', ')}`;
      md += '\n';
    });
    md += '\n';
  }

  md += `---\n\n*Generated by [Tsundoku](https://aniketbrainstorms.github.io/tsundoku)*\n`;

  const blob = new Blob([md], { type: 'text/markdown;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `tsundoku-library-${new Date().toISOString().slice(0, 10)}.md`;
  a.click();
